...
async function loadData() {
  ...
  try {
    ...
    if (!data.accounts || data.accounts.length === 0) { showEmpty(true); showContent(false); return; }
    showEmpty(false); showContent(true);
...
function populateAccountSelector(accounts) {
  const sel = document.getElementById("account-selector");
  sel.innerHTML = accounts.map((a, i) => `<option value="${a.id}">${a.building} ${a.room}号房 (${a.student_id})</option>`).join("");
...
function renderChart(history, group) {
  ...
  if (!history || history.length < 2) {
    chartEmpty.classList.remove("hidden"); canvas.style.display = "none";
    if (chart) { chart.destroy(); chart = null; }
    return;
  }
  chartEmpty.classList.add("hidden"); canvas.style.display = "";
  ...
  if (group === "raw") {
    ...
    labels = history.map(h => h.time.slice(5)); // MM-DD
    values = history.map(h => h.balance);
  } else { // month
    labels = history.map(h => h.time); // YYYY-MM
    values = history.map(h => h.balance);
  }
  ...
  chart = new Chart(document.getElementById("chart"), {
    ...
    data: {
      labels,
      datasets: [{
        label: group === "raw" ? "剩余电量 (度)" : `日均电量 (度/${group === "day" ? "天" : "月"})`,
        data: values,
        borderColor: "#4f9cf5",
    ...
    options: {
      ...
      scales: {
        x: { ticks: { color: textColor, maxTicksLimit: 12, font: { size: 10 } }, grid: { color: gridColor } },
        y: { ticks: { color: textColor, font: { size: 10 } }, grid: { color: gridColor }, title: { display: true, text: "度", color: textColor } },
      },
    },
  });
}
...
function updateChartColors() {
  ...
  const gridColor = isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)";
  const textColor = isDark ? "#aaa" : "#666";
  chart.options.scales.x.ticks.color = textColor; chart.options.scales.x.grid.color = gridColor;
  chart.options.scales.y.ticks.color = textColor; chart.options.scales.y.grid.color = gridColor;
...
function renderAccountTable(accounts) {
  document.getElementById("account-tbody").innerHTML = accounts.map(a => {
    ...
    return `<tr>
      <td>${escapeHtml(a.student_id)}</td><td>${escapeHtml(a.campus)}</td><td>${escapeHtml(a.building)}</td>
      <td>${escapeHtml(a.room)}</td>
      <td><span class="status ${a.token_valid ? 'status-ok' : 'status-bad'}">${a.token_valid ? "有效" : "过期"}</span></td>
      <td>${subText}</td><td>${balText}${dailyText}</td></tr>`;
  }).join("");
}
...
function startAutoRefresh() { if (refreshTimer) clearInterval(refreshTimer); refreshTimer = setInterval(loadData, 60000); }

await loadData();
startAutoRefresh();
window.addEventListener("beforeunload", () => { if (refreshTimer) clearInterval(refreshTimer); if (chart) chart.destroy(); });